...
COMPFLAGS="/MT /FP:precise /Dsnprintf=_snprintf /DNO_STDIO_REDIRECT /Wall /O3 /Oy /Og /EHsc"
rm -rf release-ia32-config
mkdir release-ia32-config
cd release-ia32-config
LD="cccl" LIBS="user32.lib comdlg32.lib shell32.lib" CC="cccl" CXX="cccl" RANLIB="true" CFLAGS="$COMPFLAGS" CXXFLAGS="$COMPFLAGS" ../../vpnes-0.4/configure --enable-interactive --disable-showfps --with-configfile=local --with-sdl-prefix=/home/Viktor/msvc --with-ft-prefix=/home/Viktor/msvc --with-sdl_ttf-prefix=/home/Viktor/msvc --disable-sdltest --disable-freetypetest --host=i686-w64-mingw32 --build=i686-w64-mingw32 || exit 1
